...
export const ServicesPage: React.FC<{ setPage: (p: string) => void }> = ({ setPage }) => {
  ...
  return (
    <div className="pt-32 pb-20 animate-reveal container mx-auto px-6">
      ...
      <div className="max-w-4xl mb-16 reveal">
        <div className="inline-flex items-center space-x-2 text-sky-400 mb-6 bg-sky-500/10 px-4 py-1 rounded-full border border-sky-500/20">
        ...
        <h1 className="text-5xl md:text-8xl font-black text-main mb-8 leading-tight tracking-tighter">
          Domain <span className="text-sky-400">Mastery.</span>
        </h1>
      ...
      <div className="flex flex-wrap gap-3 mb-12 reveal">
        {sectors.map((s, i) => (
          ...
          >
            <span className={active === i ? 'text-white' : 'text-sky-400'}>{s.icon}</span>
            <span>{s.name}</span>
          </button>
        ))}
      </div>
        ...
        {sectors.map((s, i) => (
          active === i && (
            <div key={i} className="animate-reveal">
              <div className="glass p-10 md:p-20 rounded-[4rem] border-sky-500/10 bg-main/10 relative overflow-hidden shadow-2xl">
                ...
                <div className="grid lg:grid-cols-12 gap-16 relative z-10">
                  ...
                  <div className="lg:col-span-7 space-y-10">
                    <div>
                      ...
                      <div className="p-6 bg-sky-500/5 rounded-3xl border border-sky-500/10 italic text-sky-400 text-sm font-bold">
                        " {s.impact} "
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {s.keywords.map((kw, kIdx) => (
                          <span key={kIdx} className="px-4 py-2 bg-main/10 rounded-xl text-[10px] font-black text-sky-500 border border-sky-500/10 uppercase tracking-widest">
                            {kw}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="lg:col-span-5 space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 gap-6">
                      {Object.entries(s.stats).map(([key, value], idx) => (
                        <ParallaxCard key={idx} className="glass p-8 rounded-3xl border-sky-500/10 bg-main/10 group shadow-lg">
                          <div className="flex items-center justify-between">
                            <div>
                              <div className="text-[10px] font-black text-muted uppercase tracking-[0.3em] mb-1">{key} Improvement</div>
                              <div className="text-4xl font-black text-sky-400 group-hover:scale-110 transition-transform origin-left">{value}</div>
                            </div>
                          ...
                          {/* Animated "data" bar */}
                          <div className="mt-6 h-1 w-full bg-main/10 rounded-full overflow-hidden">
                             <div 
      ...
      <section className="mt-48 reveal">
        <div className="text-center max-w-3xl mx-auto mb-20">
          <h2 className="text-4xl font-black text-main mb-6">Cross-Sector <span className="text-sky-400">Engineering.</span></h2>
        ...
        <div className="grid md:grid-cols-3 gap-12">
          {[
            ...
            <div key={idx} className="glass p-12 rounded-[3rem] border-sky-500/10 hover:border-sky-500/30 transition-all text-center group bg-main/5 shadow-xl">
      ...
      <section className="mt-40 p-12 md:p-24 rounded-[4rem] glass border-sky-500/10 bg-sky-500/5 text-center reveal overflow-hidden relative shadow-2xl">
         <div className="absolute top-0 left-0 w-full h-[2px] bg-sky-500/20 animate-[auditScan_5s_linear_infinite]" />
         <div className="relative z-10">
            <h2 className="text-4xl md:text-6xl font-black text-main mb-8 leading-tight">Ready to Optimize Your <br/><span className="text-sky-400">Industrial Output?</span></h2>
